            ...
            modalContent.innerHTML = `
                ...
                <div class="flex items-center justify-between mb-4 gap-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button title="批量复制播放链接" onclick="copyLinks()" class="px-3 py-1.5 bg-[#333] hover:bg-[#444] border border-[#444] rounded text-sm transition-colors flex items-center gap-1">
...
async function importConfig() {
    showImportBox(async (file) => {
        try {
            ...
            if (config.name !== 'LibreTVX-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'LibreTVX-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'LibreTVX-Settings_' + times + '.json');
}
