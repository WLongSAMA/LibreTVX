...
function initializePageContent() {
    ...
    // 设置页面标题
    document.title = currentVideoTitle + ' - LibreTVX播放器';
...
function initPlayer(videoUrl, sourceCode) {
    ...
    dp = new DPlayer({
        ...
        contextmenu: [                   // 自定义右键菜单
            {
                text: '关于 LibreTVX',
                link: 'https://github.com/WLongSAMA/LibreTVX'
            },
            {
                text: '问题反馈',
                click: (player) => {
                    window.open('https://github.com/WLongSAMA/LibreTVX/issues', '_blank');
                }
            }
        ],
        video: {
            url: videoUrl,
            type: 'hls',
            pic: 'images/nomedia.png', // 设置视频封面图
            customType: {
